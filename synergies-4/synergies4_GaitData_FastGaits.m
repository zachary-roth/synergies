...
% Get a list of indices for all the fast gait trials
fast_gaits = find(strcmp(meta.movement,'gait_fast'));

% Get a list of indices for all the valid trials (recorded ICs)
valid_ICs = find(~isnan(meta.IC1));

% Filter out the invalid fast gait trials
gait_indices = intersect(fast_gaits,valid_ICs);
