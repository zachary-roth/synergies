%{
__/\\\\____________/\\\\________________________________________________/\\\\\\\\\\__        
 _\/\\\\\\________/\\\\\\______________________________________________/\\\///////\\\_       
  _\/\\\//\\\____/\\\//\\\_____________________________________________\///______/\\\__      
   _\/\\\\///\\\/\\\/_\/\\\_____/\\\\\_____/\\\____/\\\_____/\\\\\\\\__________/\\\//___     
    _\/\\\__\///\\\/___\/\\\___/\\\///\\\__\//\\\__/\\\____/\\\/////\\\________\////\\\__    
     _\/\\\____\///_____\/\\\__/\\\__\//\\\__\//\\\/\\\____/\\\\\\\\\\\____________\//\\\_   
      _\/\\\_____________\/\\\_\//\\\__/\\\____\//\\\\\____\//\\///////____/\\\______/\\\__  
       _\/\\\_____________\/\\\__\///\\\\\/______\//\\\______\//\\\\\\\\\\_\///\\\\\\\\\/___ 
        _\///______________\///_____\/////_________\///________\//////////____\/////////_____
...
Description:
    This script is the first step in the synergies3 workflow. This script
is functionally identical to MovementData1.m, however it accomodates slight
differences in the metadata and naming conventions found in the Synergies3
data.
    ...
    Synergies2 (Dir)
        CP* (Dir for each participant)
            ...
            ID (Dir containing ID data)
                CP*_T0_(trial_ID)_MRI_IK_ID.sto
            IK
                CP*_T0_(trial_ID)_MRI_IK.mot
            Models
                *.osim
            CP*_trials.xlsx
Inputs:
...
% IMPORT DATA
%  Select the synergies3 data folder
disp('Select the synergies3 data folder')
synergiesPath = uigetdir('','Select the synergies3 data folder');
cd(synergiesPath)
...
for subj = 1:length(subjects)
    ...
    MoveData.(subjects{subj}).meta.trials = meta(valid_trials,(1:7));
    ...
    for f = 1:height(meta)
        if meta.Calc_trial(f) == 0
            ...
            Misc.IKfile = {fullfile(subjDir,'IK',strcat(meta.File{f},'_MRI_IK.mot'))};
            Misc.IDfile = {fullfile(subjDir,'ID',strcat(meta.File{f},'_MRI_IK_ID.sto'))};
